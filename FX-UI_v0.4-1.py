...
class VideoFrameExtractor(QtWidgets.QWidget):
    ...
    def create_upload_page(self):
        ...
        self.gpu_acceleration_checkbox = QtWidgets.QCheckBox(
            ...
            (" (GPU可用)" if self.has_gpu else " (GPU不可用)")
        )
        self.gpu_acceleration_checkbox.setEnabled(self.has_gpu)  # 根据GPU可用性设置是否可用
        self.gpu_acceleration_checkbox.setChecked(False)  # 默认不勾选
    ...
    def create_help_page(self):
        ...
        tutorial_text = (
            ...
            "<p>7. 可以删除不需要的帧。</p>"
            "<h2>作者简介</h2>"
            "<p>Kuie，b站主页：<a href='https://space.bilibili.com/3546720325601780?spm_id_from=333.1007.0.0'>点击这里访问</a></p>"
            "<p>用于计算机视觉图片预处理</p>"
            "<img src='path/to/your/image.png' width='300' height='200'/>"  # 插入图片
        )

        tutorial_label = QtWidgets.QTextEdit()
        tutorial_label.setHtml(tutorial_text)
        tutorial_label.setReadOnly(True)  # 设置为只读
        tutorial_label.setOpenExternalLinks(True)  # 允许打开外部链接
