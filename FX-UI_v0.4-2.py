...
class VideoFrameExtractorThread(QThread):
    ...
    def process_frame_gpu(self, frame):
        ...
        try:
            ...
            self.error.emit(f"GPU处理出错: {str(e)}")
            ...
            try:
                ...
                while True:
                    ...
                    if frame_count % interval_frames == 0:
                        if self.use_gpu:
                            frame = self.process_frame_gpu(frame)  # 使用GPU处理帧
...
class VideoFrameExtractor(QtWidgets.QWidget):
    def __init__(self):
        ...
        self.has_gpu = check_gpu_availability()  # 检查GPU可用性
        self.video_paths = []
        ...
        self.gpu_acceleration_checkbox = None
        self.fast_extraction_checkbox = None
        self.separate_folders_checkbox = None
        self.page3 = None
        ...
        # Setup UI (this will create all UI elements)
        self.initUI()

        # Apply stylesheet
        self.setStyleSheet(STYLE_SHEET)

        # Load settings after UI elements are created
        self.load_settings()
        ...
        # 设置GPU加速复选框的状态
        if self.gpu_acceleration_checkbox:
            self.gpu_acceleration_checkbox.setEnabled(self.has_gpu)  # 根据GPU可用性设置复选框是否可用

    def stop_extraction(self):
        if self.extractor_thread:
            self.extractor_thread.stop()  # 调用线程的停止方法
            self.extractor_thread.finished.connect(self.on_finished)  # 连接线程结束信号
            self.extractor_thread.quit()  # 退出线程
            self.extractor_thread = None  # 清理线程引用
        self.is_processing = False
    ...
    def load_settings(self):
        if os.path.exists(self.config_file):
            ...
            if 'Paths' not in self.config:
                self.config['Paths'] = {}
            if hasattr(self, 'output_folder_input') and self.output_folder_input is not None:
    ...
    def initUI(self):
        self.setWindowTitle('视频帧提取器 (FX-UI_0.4)')
    ...
    def create_upload_page(self):
        ...
        self.interval_slider.setMaximum(600)
        ...
        # 创建并隐藏GPU加速复选框
        self.gpu_acceleration_checkbox = QtWidgets.QCheckBox(
            "启用 GPU 加速 (Enable GPU Acceleration)"
        )
        self.gpu_acceleration_checkbox.setVisible(False)  # 隐藏复选框
        ...
        self.fast_extraction_checkbox = QtWidgets.QCheckBox("关闭图片预览 (Disable Image Preview)")
        settings_layout.addWidget(self.fast_extraction_checkbox)

        self.separate_folders_checkbox = QtWidgets.QCheckBox(
            "批量处理时，按照不同的视频保存到对应的文件夹 (Separate folders for each video)"
        )
        ...
        # 添加停止按钮
        self.stop_button = QtWidgets.QPushButton('停止 (Stop)')
        self.stop_button.clicked.connect(self.stop_extraction)  # 连接到停止槽
    ...
    def create_help_page(self):
        ...
        tutorial_text = (
            ...
            "<p>2. 设置帧间隔。</p>"
            "<p>3. 选择输出目录。</p>"
            "<p>4. 若希望在批量处理的时候，根据不同的视频将提取的帧保存在不同的文件夹，则勾选批量处理分文件夹。</p>"
            "<p>5. 点击提取帧按钮开始提取。</p>"
            "<p>6. 在预览页面中查看提取的帧，可以双击查看大图片，点击图片任意地方结束大图预览，单机勾选图片，点击删除按钮后执行删除。</p>"
            "<p>7. 可以删除不需要的帧。</p>"
            "<h2>作者简介</h2>"
            "<p>1Kuie，b站主页：<a href='https://space.bilibili.com/3546720325601780?spm_id_from=333.1007.0.0'>点击这里访问</a></p>"
            "<p>2Kuie，b站主页：https://space.bilibili.com/3546720325601780?spm_id_from=333.1007.0.0</p>"
            "<img src='微信图片_20241118174202.png' width='1200' height='400'/>"  # 插入图片
        )

        tutorial_browser = QtWidgets.QTextBrowser()
        tutorial_browser.setHtml(tutorial_text)
        tutorial_browser.setOpenExternalLinks(True)  # 允许打开外部链接
        layout.addWidget(tutorial_browser)
    ...
    def extract_frames(self):
        ...
        interval = self.interval_slider.value() / 10  # 以秒为单位
        use_gpu = self.gpu_acceleration_checkbox.isChecked()

        self.extractor_thread = VideoFrameExtractorThread(
            self.video_paths,
            output_folder,
            interval,
            use_gpu,
        ...
        # 仅在快速提取模式未选中时连接新帧提取信号
        if not self.fast_extraction_checkbox.isChecked():
